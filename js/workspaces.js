/**
 * AI Command Center - Project Workspaces Manager
 * Unifies project tools, markdown notes, prompts, and sessions in single cohesive dashboards.
 */
const WorkspaceManager = {
  workspacesList: [],

  /**
   * Initialize Workspaces
   */
  async init() {
    const data = await StorageManager.get('workspaces');
    this.workspacesList = data.workspaces || [];
    this.renderWorkspaces();
  },

  /**
   * Render Workspaces Grid
   */
  renderWorkspaces() {
    const container = document.getElementById('workspaces-grid-container');
    if (!container) return;

    if (this.workspacesList.length === 0) {
      container.innerHTML = `
        <div style="grid-column: 1 / -1; text-align: center; padding: 48px; color: var(--text-dim);">
          <p>No Project Workspaces created yet. Click "Create Workspace" to group tools and notes for a specific project.</p>
        </div>
      `;
      return;
    }

    container.innerHTML = this.workspacesList.map(ws => {
      const safeWsName = this.escapeHtml(ws.name);
      const safeWsDesc = this.escapeHtml(ws.description || 'Active project workspace');
      const safeWsId = this.escapeHtml(ws.id);

      const toolIconsHtml = (ws.toolIds || []).map(tId => {
        const tool = (typeof ToolsManager !== 'undefined' && ToolsManager.toolsList ? ToolsManager.toolsList.find(t => t.id === tId) : null) || { name: tId, iconBg: 'var(--accent-primary, #6366f1)', iconText: 'AI' };
        const safeToolName = this.escapeHtml(tool.name);
        const safeToolIcon = this.escapeHtml(tool.iconText || tool.name.slice(0, 2).toUpperCase());
        return `
          <div class="mini-tool-avatar" style="background: ${tool.iconBg || 'var(--accent-primary, #6366f1)'}; width: 28px; height: 28px; font-size: 11px;" title="${safeToolName}">
            ${safeToolIcon}
          </div>
        `;
      }).join('');

      return `
        <div class="workspace-card" draggable="true" data-id="${safeWsId}">
          <div style="display: flex; align-items: flex-start; justify-content: space-between; margin-bottom: 10px;">
            <div>
              <h3 style="font-size: 18px; margin-bottom: 4px;">${safeWsName}</h3>
              <p style="font-size: 13px; color: var(--text-muted);">${safeWsDesc}</p>
            </div>
            <div style="display: flex; gap: 6px; align-items: center;">
              <button class="tool-action-btn" data-action="edit-workspace" data-id="${safeWsId}" title="Edit Workspace">
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                  <path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"></path>
                  <path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"></path>
                </svg>
              </button>
              ${!ws.isDefault && ws.id !== 'ws-main' ? `
                <button class="tool-action-btn" data-action="delete-workspace" data-id="${safeWsId}" title="Delete Workspace">
                  <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                    <polyline points="3 6 5 6 21 6"></polyline>
                    <path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"></path>
                  </svg>
                </button>
              ` : ''}
            </div>
          </div>

          <div class="workspace-notes-box">
            <strong style="color: var(--text-highlight); font-size: 11.5px; text-transform: uppercase; letter-spacing: 0.05em; display: block; margin-bottom: 4px;">Project Notes & Goals:</strong>
            ${ws.notes ? this.escapeHtml(ws.notes) : 'No notes added yet.'}
          </div>

          <div style="margin-bottom: 18px;">
            <span style="font-size: 12px; font-weight: 600; color: var(--text-dim); display: block; margin-bottom: 8px;">Workspace Tools:</span>
            <div style="display: flex; gap: 8px; flex-wrap: wrap;">
              ${toolIconsHtml || '<span style="font-size: 12px; color: var(--text-dim);">No tools linked</span>'}
            </div>
          </div>

          <div style="display: flex; gap: 10px; margin-top: auto;">
            <button class="btn-primary" style="flex: 1; justify-content: center;" data-action="launch-workspace" data-id="${safeWsId}">
              <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                <polygon points="5 3 19 12 5 21 5 3"></polygon>
              </svg>
              Launch Workspace
            </button>
          </div>
        </div>
      `;
    }).join('');

    // Attach event listeners to prevent CSP inline event handler violations
    container.querySelectorAll('[data-action="edit-workspace"]').forEach(btn => {
      btn.addEventListener('click', (e) => {
        e.stopPropagation();
        const id = btn.getAttribute('data-id');
        if (id) this.openEditModal(id);
      });
    });

    container.querySelectorAll('[data-action="delete-workspace"]').forEach(btn => {
      btn.addEventListener('click', (e) => {
        e.stopPropagation();
        const id = btn.getAttribute('data-id');
        if (id) this.deleteWorkspace(id);
      });
    });

    container.querySelectorAll('[data-action="launch-workspace"]').forEach(btn => {
      btn.addEventListener('click', (e) => {
        e.stopPropagation();
        const id = btn.getAttribute('data-id');
        if (id) this.launchWorkspaceTools(id);
      });
    });

    // Drag-to-Reorder mechanics
    this.initDragAndDrop(container);
  },

  /**
   * HTML5 Drag-and-Drop Reordering for Workspaces
   */
  initDragAndDrop(container) {
    let draggedId = null;

    container.querySelectorAll('.workspace-card').forEach(card => {
      card.addEventListener('dragstart', (e) => {
        draggedId = card.getAttribute('data-id');
        card.style.opacity = '0.4';
        e.dataTransfer.effectAllowed = 'move';
        e.dataTransfer.setData('text/plain', draggedId);
      });

      card.addEventListener('dragend', () => {
        card.style.opacity = '1';
        container.querySelectorAll('.workspace-card').forEach(c => c.classList.remove('drag-over-target'));
      });

      card.addEventListener('dragover', (e) => {
        e.preventDefault();
        e.dataTransfer.dropEffect = 'move';
        card.classList.add('drag-over-target');
      });

      card.addEventListener('dragleave', () => {
        card.classList.remove('drag-over-target');
      });

      card.addEventListener('drop', async (e) => {
        e.preventDefault();
        card.classList.remove('drag-over-target');
        const targetId = card.getAttribute('data-id');

        if (!draggedId || draggedId === targetId) return;

        const fromIdx = this.workspacesList.findIndex(w => w.id === draggedId);
        const toIdx = this.workspacesList.findIndex(w => w.id === targetId);

        if (fromIdx !== -1 && toIdx !== -1) {
          const [movedWs] = this.workspacesList.splice(fromIdx, 1);
          this.workspacesList.splice(toIdx, 0, movedWs);

          await StorageManager.set({ workspaces: this.workspacesList });
          this.renderWorkspaces();
        }
      });
    });
  },

  /**
   * Open Edit Modal for a Workspace
   * @param {string} workspaceId
   */
  openEditModal(workspaceId) {
    const ws = this.workspacesList.find(w => w.id === workspaceId);
    if (!ws) return;

    const modal = document.getElementById('modal-add-workspace');
    const form = document.getElementById('form-add-workspace');
    if (!modal || !form) return;

    const titleEl = modal.querySelector('.modal-header h3');
    const submitBtn = form.querySelector('button[type="submit"]');

    if (titleEl) titleEl.textContent = 'Edit Project Workspace';
    if (submitBtn) submitBtn.textContent = 'Update Workspace';

    form.dataset.editId = ws.id;
    const nameInp = document.getElementById('input-ws-name');
    const descInp = document.getElementById('input-ws-description');
    const notesInp = document.getElementById('input-ws-notes');

    if (nameInp) nameInp.value = ws.name || '';
    if (descInp) descInp.value = ws.description || '';
    if (notesInp) notesInp.value = ws.notes || '';

    // Re-populate and check linked tools
    const checkboxesContainer = document.getElementById('ws-tools-checkboxes');
    if (checkboxesContainer && typeof ToolsManager !== 'undefined') {
      checkboxesContainer.innerHTML = ToolsManager.toolsList.map(t => {
        const isChecked = (ws.toolIds || []).includes(t.id) ? 'checked' : '';
        return `
          <label class="ai-checkbox-label" style="font-size: 12px; padding: 4px 8px;">
            <input type="checkbox" value="${t.id}" ${isChecked}>
            <span>${t.name}</span>
          </label>
        `;
      }).join('');
    }

    UI.openModal('modal-add-workspace');
  },

  /**
   * Update existing workspace
   * @param {string} workspaceId
   * @param {object} wsData
   */
  async updateWorkspace(workspaceId, wsData) {
    const index = this.workspacesList.findIndex(w => w.id === workspaceId);
    if (index === -1) {
      UI.showToast('Workspace not found', 'error');
      return false;
    }

    if (!wsData.name) {
      UI.showToast('Please provide a Workspace Name', 'error');
      return false;
    }

    this.workspacesList[index] = {
      ...this.workspacesList[index],
      name: wsData.name.trim().slice(0, 80),
      description: wsData.description ? wsData.description.trim().slice(0, 300) : '',
      notes: wsData.notes ? wsData.notes.trim().slice(0, 5000) : '',
      toolIds: Array.isArray(wsData.toolIds) ? wsData.toolIds.slice(0, 30) : this.workspacesList[index].toolIds
    };

    await StorageManager.set({ workspaces: this.workspacesList });
    this.renderWorkspaces();
    UI.showToast(`Updated workspace "${this.workspacesList[index].name}"!`, 'success');
    return true;
  },

  /**
   * Launch all tools of a workspace
   * @param {string} workspaceId
   */
  launchWorkspaceTools(workspaceId) {
    const ws = this.workspacesList.find(w => w.id === workspaceId);
    if (!ws || !ws.toolIds) return;

    ws.toolIds.forEach(toolId => {
      if (typeof ToolsManager !== 'undefined' && ToolsManager.launchTool) {
        ToolsManager.launchTool(toolId);
      }
    });

    UI.showToast(`Launched Workspace "${ws.name}" (${ws.toolIds.length} tools)!`, 'success');
  },

  /**
   * Create new workspace
   * @param {object} wsData
   */
  async createWorkspace(wsData) {
    if (!wsData.name) {
      UI.showToast('Please provide a Workspace Name', 'error');
      return false;
    }

    const newWs = {
      id: 'custom-ws-' + Date.now(),
      name: wsData.name.trim().slice(0, 80),
      description: wsData.description ? wsData.description.trim().slice(0, 300) : '',
      notes: wsData.notes ? wsData.notes.trim().slice(0, 5000) : '',
      toolIds: Array.isArray(wsData.toolIds) ? wsData.toolIds.slice(0, 30) : ['chatgpt', 'cursor', 'perplexity'],
      isDefault: false
    };

    this.workspacesList.push(newWs);
    await StorageManager.set({ workspaces: this.workspacesList });
    this.renderWorkspaces();
    UI.showToast(`Workspace "${newWs.name}" created!`, 'success');
    return true;
  },

  /**
   * Delete workspace
   * @param {string} wsId
   */
  async deleteWorkspace(wsId) {
    const ws = this.workspacesList.find(w => w.id === wsId);
    const wsName = ws ? ws.name : 'this workspace';

    const ok = await UI.confirm({
      title: 'Delete Workspace',
      message: `Are you sure you want to delete workspace "${wsName}"?`,
      confirmText: 'Delete Workspace',
      danger: true
    });

    if (ok) {
      this.workspacesList = this.workspacesList.filter(w => w.id !== wsId);
      await StorageManager.set({ workspaces: this.workspacesList });
      this.renderWorkspaces();
      UI.showToast('Workspace removed', 'info');
    }
  },

  /**
   * Escape HTML special characters to prevent XSS injection
   * @param {string} str
   * @returns {string}
   */
  escapeHtml(str) {
    if (!str) return '';
    return String(str)
      .replace(/&/g, '&amp;')
      .replace(/</g, '&lt;')
      .replace(/>/g, '&gt;')
      .replace(/"/g, '&quot;')
      .replace(/'/g, '&#039;');
  }
};
